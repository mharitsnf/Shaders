...
vec2 voronoi(vec2 i_st, vec2 f_st, float t) {
    ...
    for (int y = -2; y <= 2; y++) {
        ...
        if (l > 0.00001)
        m_dist = min(m_dist, dot( 0.5*(md+d), normalize(d-md) ));
    }
...
void main() {
	...
	float vorx = 1.- smoothstep(.02, 0.122, vor.x);
    float mask = mix(vorx, q, 0.556);
    ...
    vec3 color = mix(blue2, blue1, mask * 2.);
    vec3 color2 = mix(db1, green1, mask * mask * 3.);
    vec3 fcol = mix(color2, color, .5);
